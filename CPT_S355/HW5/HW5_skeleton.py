...
def psfor():
    if len(opstack) >= 4: 
        ...
        initial = opPop()
        if incre != 0:
            pos_neg_offset = int(incre*abs(1/incre))
            for i in range(initial, final+ pos_neg_offset*1, incre):
                opPush(i)
                interpretSPS(code_array)
        else:
            print("increment value = 0 in --psfor--")    
    else:
        print("Not enought element in --psfor--") 
...
def interpretSPS(code): # code is a code array
    for items in code:
        if type(items) is str: 
            if items[0] == '/' or (items[0] == '(' and items[-1] ==')'):
                opPush(items)
            elif items in functDict.keys():
                functDict[items]()
            else:  #items = n
                u = lookup(items)
                if type(u) is list:
